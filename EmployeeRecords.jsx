...
import { EmployeeRecord } from "@/api/entities";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { Users, Search, Calendar, Building, RefreshCw } from "lucide-react";
      ...
      <div className="p-6 md:p-8 max-w-7xl mx-auto">
            ...
            <div>
              <div className="flex items-center gap-3 mb-2">
                <div className="p-2 rounded-xl bg-gradient-to-br from-[#00d4ff] to-[#0ea5e9]">
                  <Building className="w-6 h-6 text-[#0a0a0a]" />
                </div>
              ...
              <p className="text-[#a3a3a3] text-lg">
                View and manage imported employee data and survey responses
              </p>
            </div>
        ...
        <div className="mb-6">
          <div className="relative">
            ...
            <Input
              placeholder="Search by subject ID, email, name, department, or role..."
              value={searchTerm}
          ...
          <Card className="bg-[#111111] border-[#262626]">
            <CardContent className="p-4">
              <div className="text-2xl font-bold text-[#10b981]">
                {new Set(records.map(r => r.department).filter(Boolean)).size}
              </div>
              <div className="text-sm text-[#a3a3a3]">Departments</div>
            </CardContent>
          </Card>
          <Card className="bg-[#111111] border-[#262626]">
            <CardContent className="p-4">
              <div className="text-2xl font-bold text-[#fbbf24]">
                {records.filter(r => r.survey_responses && r.survey_responses.length > 0).length}
              </div>
              <div className="text-sm text-[#a3a3a3]">With Responses</div>
            </CardContent>
          </Card>
          <Card className="bg-[#111111] border-[#262626]">
            <CardContent className="p-4">
          ...
          <CardContent className="p-0">
            {isLoading ? (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-2 flex-wrap">
                          <h3 className="font-semibold text-white">
                            {record.name || record.subject_id || 'Unknown Employee'}
                          </h3>
                          {record.department && (
                        ...
                        {record.email && (
                          <div className="text-[#a3a3a3] text-sm mb-2">
                            {record.email}
                          </div>
                        )}
                        {record.notes && (
                        ...
                        <div className="flex items-center gap-4 text-xs text-[#6b7280] flex-wrap">
                          <div className="flex items-center gap-1">
                            ...
                            Hired: {safeFormatDate(record.hired_at)}
                          </div>
                          {record.survey_responses && record.survey_responses.length > 0 && (
                            <div>
                              {record.survey_responses.length} survey response(s)
                            </div>
                          )}
              ...
              <div className="p-8 text-center">
                <Building className="w-12 h-12 text-[#a3a3a3] mx-auto mb-4" />
                <p className="text-[#a3a3a3]">
                  {searchTerm ? 'No records match your search' : 'No employee records imported yet'}
                </p>
                {!searchTerm && (
                  <p className="text-[#6b7280] text-sm mt-2">
                    Go to Data Import → Employee Records to import your first employee data
                  </p>
                )}
              </div>
            )}
